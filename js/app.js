...
function numberGame() {
  ...
  while (guessCount < 4) {
    ...
    if (guess6 < answer6) {
      guessCount++;
      alert('Sorry That is a little low, Try again. You have ' + (4 - guessCount) + ' guesses left');
    }
    else if (guess6 > answer6) {
      guessCount++;
      alert('Sorry That is a little high, Try again. You have ' + (4 - guessCount) + ' guesses left');
    }
    else if (guess6 === answer6) {
      guessCount++;
...
function bakedGoods() {
  ...
  while (!answer7valid && q7guess > 0) {
    ...
    if (!answer7valid) {
      ...
      if (q7guess === 0) {
        alert(`Sorry you didn't guess it correctly, I like these baked goods: ${favoriteBakedGoods.join(', ')}`);
        break;
      }
    }
    else if (answer7valid) {
      count++;
      alert(`You guessed it correctly! It only took you ${q7guess} guesses. My whole list of favorite baked goods are ${favoriteBakedGoods.join(', ')}`);
      break;
    }
  }
}
